...
- Adapted for Binance Futures (originally Phemex)
'''
...
from credentials.api_secrets import BINANCE_API_KEY, BINANCE_SECRET_KEY
...
markets = exchange.fetch_markets(params=params)
...
def kill_switch_all():
    ...
    for x in openpos_list:
        ...
        while openposi == True:
            ...
            if long == False:
                exchange.create_limit_buy_order(sym, kill_size, bid, params)
                print(f'** BUY to CLOSE - {sym}')
                #print('sleeping for 30 seconds to see if it fills..')
                #time.sleep(20)
            elif long == True:
                exchange.create_limit_sell_order(sym, kill_size, ask,params )
...
## FIGURE OUT IF IN POSITION IS T OR F
    ...
    if sig4hr == 'BUY':
        ...
        if ((bid <= sma15m) and (in_pos == False) and (lcBpc5m == True)):
            ...
            exchange.cancel_all_orders(symbol)
            exchange.create_limit_buy_order(symbol, pos_size, sma15m_minus008, params)
        ...
        if ((bid >= sma15m) and (in_pos == False) and (lcBpc5m == False)):
            ...
            exchange.cancel_all_orders(symbol)
            exchange.create_limit_sell_order(symbol, pos_size, sma15m_plus008, params)
...
# now we have every contract ticker on exchange, now what
...
# get all tickers and submit an order to activate all
    ...
    #     exchange.create_limit_buy_order(one_sym_pertime,pos_size,bid, params )
    #     time.sleep(2)
    #     print('---')